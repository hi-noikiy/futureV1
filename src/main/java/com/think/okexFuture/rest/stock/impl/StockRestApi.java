package com.think.okexFuture.rest.stock.impl;
...
public class StockRestApi implements IStockRestApi {
	...
	/**
	 * 现货 获取OKEx币币K线数据(每个周期数据条数2000左右)的信息URL
	 */
	...
	public String depth(String symbol) throws HttpException, IOException, InterruptedException {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpGet(url_prex, this.DEPTH_URL, param);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String trades(String symbol, String since) throws HttpException, IOException, InterruptedException {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpGet(url_prex, this.TRADES_URL, param);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String userinfo() throws HttpException, IOException, InterruptedException {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.USERINFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String trade(String symbol, String type,
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.TRADE_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String batch_trade(String symbol, String type,
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.BATCH_TRADE_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String cancel_order(String symbol, String order_id) throws HttpException, IOException, InterruptedException {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.CANCEL_ORDER_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String order_info(String symbol, String order_id) throws HttpException, IOException, InterruptedException {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.ORDER_INFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	@Override
	public String orders_info(String type, String symbol, String order_id) throws HttpException, IOException, InterruptedException {
		// 构造参数签名
		Map<String, String> params = new HashMap<String, String>();
		params.put("api_key", api_key);
		if (!StringUtil.isEmpty(type)) {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.ORDERS_INFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String order_history(String symbol, String status, String current_page, String page_length)
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.ORDER_HISTORY_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	@Override
	public String kline(String symbol, String type, String size, String since) throws HttpException, IOException, InterruptedException {
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String param = "";
		if (!StringUtil.isEmpty(symbol)) {
			if (!param.equals("")) {
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpGet(url_prex, this.KLINE_URL, param);
				Thread.sleep(200);
			} else {
				break;
			}
