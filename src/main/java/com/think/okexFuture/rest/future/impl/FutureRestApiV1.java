package com.think.okexFuture.rest.future.impl;
...
import com.think.okexFuture.util.HttpUtilManager;
import com.think.okexFuture.util.MD5Util;
...
public class FutureRestApiV1 implements IFutureRestApi {
	...
	private static Logger log = Logger.getLogger(FutureRestApiV1.class);
	 ...
	 * 获取虚拟合约的K线数据
	 */
	private static final String FUTURE_KLINE_URL = "/api/v1/future_kline.do";
	 ...
	 * 批量获取合约订单信息
	 */
	private final String FUTURE_ORDERS_INFO_URL = "/api/v1/future_orders_info.do";
	...
	public String future_ticker(String symbol, String contractType)
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
	...
	public String future_cancel(String symbol, String contractType,
		String orderId) throws HttpException, IOException, InterruptedException {
		// 构造参数签名
	...
	public String future_trade(String symbol, String contractType,
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_TRADE_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_userinfo() throws HttpException, IOException, InterruptedException {
		...
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String result = httpUtil.requestHttpPost(url_prex, this.FUTURE_USERINFO_URL, params);
		// System.out.println(result);
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_USERINFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_userinfo_4fix()
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_USERINFO_4FIX_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_position(String symbol, String contractType)
		...
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String result = httpUtil.requestHttpPost(url_prex, this.FUTURE_POSITION_URL, params);
		// System.out.println(result);
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_POSITION_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_position_4fix(String symbol, String contractType)
		...
		// 发送post请求
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String result = httpUtil.requestHttpPost(url_prex, this.FUTURE_POSITION_4FIX_URL, params);
		// System.out.println(result);
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_POSITION_4FIX_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_order_info(String symbol, String contractType,
		...
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_ORDER_INFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	@Override
	public String future_orders_info(String symbol, String contractType,
		String orderId)
		throws HttpException, IOException, InterruptedException {
		// 构造参数签名
		Map<String, String> params = new HashMap<String, String>();
		...
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String result = httpUtil.requestHttpPost(url_prex, this.FUTURE_ORDERS_INFO_URL, params);
		// System.out.println(result);
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
				result = httpUtil.requestHttpPost(url_prex, this.FUTURE_ORDERS_INFO_URL, params);
				Thread.sleep(200);
			} else {
				break;
			}
	...
	public String future_kLine(String symbol, String type, String contractType, String size, String since)
		...
		String result = httpUtil.requestHttpGet(url_prex, FUTURE_KLINE_URL, param);
		while (true) {
			if (result.toLowerCase().contains("bad gateway")) {
