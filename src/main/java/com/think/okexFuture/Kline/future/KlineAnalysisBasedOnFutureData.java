...
public class KlineAnalysisBasedOnFutureData {

	//default 波动容忍值为0.002，也就是波动在0.2%的不考虑重置为0
	private static double tolerantRange = 0.004;
	private static IFutureRestApi futureGetV1 = new FutureRestApiV1(OkexProperties.URL_PREX);
	private static Logger log = Logger.getLogger(KlineAnalysisBasedOnFutureData.class);
	...
	public static boolean historyGoingUpExpectedTimes(String preAnalysisData, int expectedTimes, double tolerantRange) {
		List<String[]> formatResult = StringUtil.formatKlineDataWithoutLatest(preAnalysisData);
		int count = 0;
		...
		log.info("result size:" + formatResult.size() + " historyGoingUp expected Times: " + expectedTimes + " count: " + count);
	...
	public static boolean historyGoingDownExpectedTimes(String preAnalysisData, int expectedTimes, double tolerantRange) {
		...
		log.info("result size:" + formatResult.size() + " historyGoingDown expected Times: " + expectedTimes + " count: " + count);
		if (count >= expectedTimes) {
			return true;
		} else {
			return false;
	...
	//时间未到会sleep
	public static boolean current15MinAnalysisWillUp(String symbol, String contractType)
		...
		while (true) {
			String current15minresult = futureGetV1.future_kLine(symbol, "15min", contractType, "1", "");
			List<String[]> temp = StringUtil.formatKlineData(current15minresult);
			...
			if (timeback < 10) {
				...
				String result = futureGetV1.future_kLine(symbol, "1min", contractType, String.valueOf(timeback), "");
				List<String[]> resultList = StringUtil.formatKlineData(result);
				...
				if (count > 9) {
					...
					log.info("current count less than expected 9, current count: " + count + " wait for 30 seconds to continue");
					count = 0;
					Thread.sleep(30000);
					continue;
				}
	...
	//时间未到会sleep
	public static boolean current15MinAnalysisWillDown(String symbol, String contractType)
		throws HttpException, IOException, InterruptedException, ParseException {
	...
	public static void main(String[] args) throws HttpException, IOException, InterruptedException, ParseException {
		...
		String preAnalysisData = futureGetV1.future_kLine("eos_usd", "15min", "this_week", "5", "");
		System.out.println(preAnalysisData);
		boolean currentUpFlag = historyGoingDownExpectedTimes(preAnalysisData, 4);
		System.out.println(currentUpFlag);
		System.out.println(current15MinAnalysisWillDown("eos_usd", "this_week"));

		System.out.println(current15MinAnalysisWillUp("eos_usd", "this_week"));
	}
