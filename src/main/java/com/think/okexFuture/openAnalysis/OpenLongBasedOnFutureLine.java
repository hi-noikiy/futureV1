 ...
 *
 */
public class OpenLongBasedOnFutureLine {
	private static Logger log = Logger.getLogger(OpenLongBasedOnFutureLine.class);
	 ...
	 */
	public static boolean isOpenLongTiming(String symbol, String type, String contractType, String size, String since, int expectedTimes)
		...
		boolean upFlag = KlineAnalysisBasedOnFutureData.historyGoingDownExpectedTimes(result, expectedTimes);
		if (upFlag) {
			log.info("15min history data, going down " + expectedTimes + " times, If current 15min is going up. I will open long+++++");
			boolean currentTrendencyFlag = KlineAnalysisBasedOnFutureData.current15MinAnalysisWillUp(symbol, contractType);
