 ...
 *
 */
public class CloseLongStrategy {

	private static Logger log = Logger.getLogger(CloseLongStrategy.class);
	 ...
	 * @return
	 */
	public static boolean isCloseLongTimingBasedOnProfit(String symbol, String contractType, Double openLongPrice, String openTimeStamp)
		...
		double profit = (openLongPrice - avg) / openLongPrice * leverage;

		if (profit > lossPercentage) {
			log.info("loss reach: " + lossPercentage + "，need to sell " + " openLongPrice: " + openLongPrice + " current AVG: " + avg
				+ " loss percentage: ：" + profit);
			return true;
		} else if (-profit > profitPercentage) {
			log.info("profit reach: " + profitPercentage + "，need to sell " + " openLongPrice: " + openLongPrice + " current AVG: " + avg
				+ " profit percentage：" + -profit);
			return true;
		} else {
			log.info(
				"not reach selling threshhold: " + " openLongPrice: " + openLongPrice + " current AVG: " + avg + " profit percentage: " + -profit);
			return false;
		}
	}

	public static void main(String[] args) throws HttpException, IOException, InterruptedException {
		String result = futureGetV1.future_orders_info("eos_usd", "this_week", "439770376645632");
		//String result = futureGetV1.future_order_info("eos_usd", "this_week", "-1", "2", "1", "1");
		System.out.println(result);
		JSONObject tempObject = JSONObject.parseObject(result);
		String resultFlag = tempObject.getString("result");
		System.out.println(resultFlag);


		String orders = tempObject.getString("orders");
		String tempString = orders.replaceAll("\\[", "").replaceAll("]", "");
		JSONObject orderInfo = JSONObject.parseObject(tempString);
