 ...
 *
 */
public class OpenShortBasedOnStockLine {
	private static Logger log = Logger.getLogger(OpenShortBasedOnStockLine.class);
	private static IStockRestApi stockGetV1 = new StockRestApi(OkexProperties.URL_PREX);
	private static int expectedTimes = 5;
	 ...
	 */
	public static boolean isOpenShortTiming(String symbol, String type, String size, String since, int expectedTimes)
		...
		boolean downFlag = KlineAnalysisBasedOnStockData.historyGoingUpExpectedTimes(result, expectedTimes);
		if (downFlag) {
			log.info("15min history data, going up " + expectedTimes + " times, If current 15min is going down. I will open short+++++");
			boolean currentTrendencyFlag = KlineAnalysisBasedOnStockData.current15MinAnalysisWillDown(symbol);
