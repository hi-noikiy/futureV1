...
import com.think.okexFuture.Kline.future.KlineAnalysisBasedOnStockData;
 ...
 *
 */
public class OpenLongBasedOnStockLine {
	private static Logger log = Logger.getLogger(OpenLongBasedOnStockLine.class);
	 ...
	 */
	public static boolean isOpenLongTiming(String symbol, String type, String size, String since, int expectedTimes)
		throws HttpException, IOException, InterruptedException, ParseException {
		String result = stockGetV1.kline(symbol, type, size, since);
		boolean upFlag = KlineAnalysisBasedOnStockData.historyGoingDownExpectedTimes(result, expectedTimes);
		if (upFlag) {
			log.info("15min history data, going down " + expectedTimes + " times, If current 15min is going up. I will open long+++++");
			boolean currentTrendencyFlag = KlineAnalysisBasedOnStockData.current15MinAnalysisWillUp(symbol);
			if (currentTrendencyFlag) {
				log.info("It's open long time, open long +++++++++++++++++++++");
				return true;
			} else {
				log.info("15min history data, going down " + expectedTimes + " times, currentTrendencyFlag is still false, waiting");
			}
		}
		return false;
	}
